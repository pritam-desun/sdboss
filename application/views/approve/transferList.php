                ...
                <div class="table-responsive mb-4 mt-4">
                    <table id="dataTable" class="table table-hover non-hover" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <?php
							if($request->status == 'P'){
								$url = base_url('approve/moneyTransfer/').$request->id.'/'.$request->cust_code;
								$className = 'warning';
                            ...
                            <tr>
                                <td>
                                    <a href="<?= $url ?>" class="btn btn-<?= $className ?>" <?= $onclick ?>>
                                        <?= $btnTtext ?> </a>
                                        <?php if($reject){ ?>
                                            <a href="<?= base_url('approve/reject/').$request->id.'/'.$request->cust_code; ?>" class="btn btn-danger" style="margin-top: 10px" onclick="return confirm(`Are you sure you want to cancel?`)">
                                       Cancel</a>
                                            <?php } ?>
                                </td>
                                <td><?= $request->full_name ?></td>
                                <td><?= $request->amount ?></td>
                                <td><?= $request->txn_ref_id ?></td>
                                <td><?php foreach($pay_types as $key => $value){ ?>
